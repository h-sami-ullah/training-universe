...
async def deploy_model(
    s3_model_uri: str = Query(..., description="s3_uri of the model to deploy")
):
    ...
    try:
        ...
        return {
            "message": f"Model deployed successfully. Available model: {os.path.basename(local_file_path)}"
        }
...
async def predict_file(
    file: UploadFile = File(..., description="CSV input file"),
    s3_model_uri: Optional[str] = Form(
        None,
        description="s3_uri of the model if the predictions "
        "are needed from a specific model",
    ),
    s3_output_uri: Optional[str] = Form(
        None, description="s3_uri if the output needs to be saved " "in S3"
    ),
    skip_feature_generator: bool = Form(
        False, description="Flag to skip feature " "extractor if already " "processed"
    ),
):
    try:
        s3_handler = S3handler()
        if s3_model_uri:
            ...
            model_file_path = os.path.join(
                temp_model_folder, os.path.basename(model_key)
            )
        else:
            if not os.path.exists("models"):
                raise HTTPException(
                    status_code=400,
                    detail="The system cannot find the path specified: 'models', "
            ...
            if not model_files:
                raise HTTPException(
                    status_code=400,
                    detail="There is no model deployed, First deploy the model.",
                )
        ...
        if s3_output_uri:
            ...
            logging.info(
                f"Combined DataFrame uploaded to S3 bucket '{s3_output_uri}' as {csv_file_name}"
            )
