...
class FeatureExtractor:
    ...
    def calc_dist(
        self,
        p1x: Union[float, np.ndarray],
        p1y: Union[float, np.ndarray],
        p2x: Union[float, np.ndarray],
        p2y: Union[float, np.ndarray],
    ...
    def add_dist_to_restaurant_feature(self) -> None:
        ...
        self.df["dist_to_restaurant"] = self.calc_dist(
            self.df.courier_lat,
            self.df.courier_lon,
            self.df.restaurant_lat,
            self.df.restaurant_lon,
        )
    ...
    def avg_dist_to_restaurants(self, courier_lat: float, courier_lon: float) -> float:
        ...
        """
        distances = [
            self.calc_dist(v["lat"], v["lon"], courier_lat, courier_lon)
    ...
    def add_haversine_dist_to_restaurant(self) -> None:
        ...
        self.df["Hdist_to_restaurant"] = self.calc_haversine_dist(
            self.df.courier_lat.tolist(),
    ...
    def avg_Hdist_to_restaurants(self, courier_lat: float, courier_lon: float) -> float:
        ...
        """
        distances = [
            self.calc_haversine_dist(courier_lat, courier_lon, v["lat"], v["lon"])
            for v in self.restaurants_ids.values()
        ]
        return np.mean(distances)
    ...
    def add_avg_haversine_distance_feature(self) -> None:
        ...
        self.df["avg_Hdist_to_restaurants"] = [
            self.avg_Hdist_to_restaurants(lat, lon)
            for lat, lon in zip(self.df.courier_lat, self.df.courier_lon)
        ]
    ...
    def initiate_centroids(self, k) -> pd.DataFrame:
        ...
        df_restaurants = pd.DataFrame(
            [{"lat": v["lat"], "lon": v["lon"]} for v in self.restaurants_ids.values()]
        )
        centroids = df_restaurants.sample(k)
        return centroids

    def centroid_assignation(
    ...
    def add_timestamp_to_datetime(self, column_name: str) -> None:
        ...
        data1 = pd.to_datetime(
            self.df[column_name], format="%Y-%m-%dT%H:%M:%S.%fZ", errors="coerce"
        )
    ...
    def add_orders_busyness_by_h3_hour_feature(self) -> None:
        ...
        index_list = [
            ...
            for (i, d, hr) in zip(
                self.df["h3_index"],
                self.df["date_day_number"],
                self.df["date_hour_number"],
            )
        ]
    ...
    def add_restaurants_per_index_feature(self) -> None:
        ...
        self.df["restaurants_per_index"] = [
            restaurants_counts_per_h3_index[h] for h in self.df["h3_index"]
        ]
...
if __name__ == "__main__":
    ...
    output_file_path = (
        r"C:\Users\Hafiz\Downloads\Laptop\Personal\Notebook\output_dataframe.csv"
    )
